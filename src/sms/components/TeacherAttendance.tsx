...
const TeacherAttendance: React.FC<TeacherAttendanceProps> = ({ schoolId }) => {
  ...
  const downloadAttendancePDF = async () => {
    ...
    try {
      ...
      doc.text('TEACHER / STAFF ATTENDANCE REPORT', 14, 28);
      ...
      doc.text(`Type: Staff daily attendance`, 14, y + 6);
      doc.text(`Date: ${fmtDate(selectedDate)}`, 14, y + 12);

      const timestamp = new Date().toLocaleString();
      doc.text(`Generated: ${timestamp}`, 135, y + 6);
      doc.text(`Total Staff: ${teachers.length}`, 135, y + 12);
      ...
      doc.text(`Present: ${stats.present} (${teachers.length ? Math.round((stats.present / teachers.length) * 100) : 0}%)`, 20, y + 10);

      doc.setTextColor(239, 68, 68); // rose-600
      doc.text(`Absent: ${stats.absent} (${teachers.length ? Math.round((stats.absent / teachers.length) * 100) : 0}%)`, 65, y + 10);

      doc.setTextColor(245, 158, 11); // amber-600
      doc.text(`Late: ${stats.late} (${teachers.length ? Math.round((stats.late / teachers.length) * 100) : 0}%)`, 110, y + 10);
      ...
      const tableRows = teachers.map((t, idx) => {
        ...
        if (t.status === 'P') statusText = 'Present';
        else if (t.status === 'A') statusText = 'Absent';
        else if (t.status === 'L') statusText = 'Late';
      ...
      autoTable(doc, {
        startY: y + 24,
        head: [['No.', 'Teacher / Staff Name', 'Status']],
        body: tableRows,
        theme: 'striped',
        headStyles: {
          fillColor: [79, 70, 229],
      ...
      doc.save(`teacher_attendance_${fmtDate(selectedDate)}.pdf`);
      showToast('Teacher Attendance PDF downloaded.', 'success');
    } catch (err: any) {
      console.error(err);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        {teachers.length > 0 && (
          <div className="flex flex-wrap gap-3">
            <button type="button" onClick={() => void downloadAttendancePDF()} disabled={isLoading || isSaving}
