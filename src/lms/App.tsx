...
const App: React.FC<AppProps> = ({ onSwitch, schoolId, schoolName, onSchoolIdChange }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row bg-[#f3f0e8] min-h-screen text-white relative">
      ...
      <header className="md:hidden flex items-center justify-between p-4 bg-[#0a1a19]/95 backdrop-blur-xl shadow-lg border-b border-white/10 sticky top-0 z-[50]">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-black tracking-tighter text-white uppercase">{schoolName || 'IEM'}</h1>
        </div>
        <button aria-label="Toggle Menu" type="button" onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="text-white p-2 hover:text-[#4ea59d] transition-colors">
      ...
      />
      <main className={`flex-1 transition-all duration-300 p-4 sm:p-6 md:p-8 overflow-x-hidden bg-[#f3f0e8] ${isSidebarOpen ? 'hidden md:block' : 'block'} ${isSidebarCollapsed ? 'md:ml-20' : 'md:ml-72'}`}>
